<template>
    <div id="container">
        <el-page-header icon="el-icon-arrow-left" style="padding:25px;" :content="GetProductName" @click="Back()" />
        <div v-if="Model != null" id="product-data-container">
            ...
            <div id="info-button-actions-container">
                ...
                <div id="size-container">
                    <button v-for="item in Model.Size" :key="item" class="custom-button-border-plain button-square" :style="ChoosenSizeCss(item)" @click="SetChoosenSize(item)">{{ item }}</button>
                </div>
                ...
                <div class="action-button">
                    <el-popconfirm
                      ...
                      icon-color="green"
                      title="Are you sure to add this to cart?"
                      @confirm="AddToCart()"
                    >
                      <template #reference>
                    ...
                    <el-popconfirm
                      ...
                      icon-color="green"
                      title="Are you sure to add this to wish list?"
                      @confirm="AddToWishListMessage(this.Model.ProductName)"
                    >
                      <template #reference>
...
import ShowAddToCartSuccess from "../components/Show-add-to-cart-success.vue";
import RouteCommand from '../mixin/Router-command.mixin';
import PopupMessage from '../mixin/Popup-message.mixin';
...
export default defineComponent({
  ...
  methods: {
      ...
      ChoosenSizeCss(Size){
        return {
          backgroundColor: this.ChoosenSize == Size? "#2EE59D" : "white",
          color: this.ChoosenSize == Size? "white" : "#2EE59D",
          marginRight: "10px",
          width: "120px"
        };
      },
      AddToCart(){
        if(this.ChoosenSize == "" || this.ChoosenSize == null){
          this.$message.error('Please choose size.');
          return;
        }
        ...
        const OrderItem = {
          ...
          Color: this.ChoosenColor.toLowerCase(),
          Price: this.Model.Price,
          Size: this.ChoosenSize
        } as CartModel;

        if(new CartService().AddToCart(OrderItem)){
          this.AddToCartMessage(this.Model.ProductName);
        }else{
          this.$alert('Encounter an error saving to cart, please contact admin.', 'ERROR', {
  ...
  computed: {
    GetProductName(){
      return this.Model == null? "Please wait" : this.Model.ProductName;
    }
  },
  data() {
      return {
